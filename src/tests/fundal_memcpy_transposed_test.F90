!< FUNDAL, transposed device memory copy test with lower bound -2.

#include "fundal.H"

program fundal_memcpy_transposed_test
!< FUNDAL, transposed device memory copy test with lower bound -2.
!< Tests dev_memcpy_from_device with transposed (dims 1 and 2 swapped) host layout,
!< for device arrays with lower bound -2 on every dimension, ranks 2-7.
...
integer(I4P)            :: n                   !< Upper bound of arrays (lower bound is -2).
...
contains
   ...
   character(10) :: nstr !< Upper bound of arrays, stringified.
...
#include "fundal_memcpy_transposed_test_agnostic.INC"
...
#include "fundal_memcpy_transposed_test_agnostic.INC"
endprogram fundal_memcpy_transposed_test
